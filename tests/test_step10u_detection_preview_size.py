...
def test_image_input_preserves_pre_resize_display_preview(tmp_path):
...
def test_detection_runtime_prefers_pre_resize_display_preview(monkeypatch):
    monkeypatch.setattr(
        runtime,
        "decode_detection_predictions",
        lambda *args, **kwargs: [torch.tensor([[0.1, 0.2, 0.8, 0.9, 0.95, 1.0]])],
    )
    model = _Detector()
...
def test_detection_frontend_uses_large_stage_and_source_aspect_ratio():

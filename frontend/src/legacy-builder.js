function __MLB_STUDIO_FACTORY__(){
  ...
  function mount(root,payload){
    ...
    const reactInstanceId=String(payload.instance_id||root.id||"mlb-studio");
    ...
    let galleryWorkspace={open:false,tab:"core"};
    ...
    let bridgeDocumentsCache={};
    let bridgeDocumentsCacheAt={};
    ...
    function openGallery(tab){
      ...
      cloudWorkspace.open=false;
      bottomExpanded=false;
      const requested=(typeof tab==="string")?tab:(state.active_workspace==="data"?"data":"core");
      const normalized=requested==="mine"?"mine":requested;
      galleryWorkspace={open:true,tab:["core","models","mine","components","data","drafts"].includes(normalized)?normalized:(state.active_workspace==="data"?"data":"core")};
      outputDirectorySelection=null;
      selected=null;
    ...
    function trainingLocksGeneration(entry){
      entry=liveBuiltModel(entry)||entry;
      if(trainingIsRunning())return true;
      const liveStatus=String(entry?.training_live?.status||"").toLowerCase();
      return liveStatus==="starting"||liveStatus==="running"||liveStatus==="stopping";
    }
    ...
    function startGenerationFromRuntime(entry){
      ...
      setTimeout(()=>requestRuntimeCommand("generate",entry),20);
    }
    ...
    function bridgeDocuments(force=false,includeFrames=false){
      const now=Date.now();
      const cacheKey=includeFrames?"frames":"direct";
      const cached=bridgeDocumentsCache?.[cacheKey];
      const cachedAt=bridgeDocumentsCacheAt?.[cacheKey]||0;
      if(!force && cached && (now-cachedAt)<4000)return cached;
      ...
      // Notebook startup must never enumerate every iframe. Frame enumeration is
      // reserved for an explicit user action when the fast direct lookup failed.
      if(includeFrames){
        const parents=[...docs];
        parents.forEach(doc=>{
          try{
      ...
      if(!bridgeDocumentsCache||typeof bridgeDocumentsCache!=="object")bridgeDocumentsCache={};
      if(!bridgeDocumentsCacheAt||typeof bridgeDocumentsCacheAt!=="object")bridgeDocumentsCacheAt={};
      bridgeDocumentsCache[cacheKey]=docs;
      bridgeDocumentsCacheAt[cacheKey]=now;
      return docs;
    }
    ...
    function canvasScrollKey(){
      if(galleryWorkspace.open)return "gallery:"+(galleryWorkspace.tab||"core");
    ...
    function bridgeRoot(cls,force=false,allowDeep=false){
      if(!cls)return null;
      const cached=bridgeHostCache.get(cls);
      if(!force && cached && cached.isConnected!==false)return cached;
      if(force)bridgeHostCache.delete(cls);
      const selector="."+cls;

      // Fast path: only current/parent/top documents. This is the only lookup
      // allowed during Studio startup and background polling.
      for(const doc of bridgeDocuments(force,false)){
        try{
      ...
      // Expensive fallback is user-triggered only. It may inspect notebook
      // frames/shadow roots, but never runs while the dashboard is merely loading.
      for(const doc of bridgeDocuments(true,true)){
    ...
    function bridgeControl(cls,selector,force=false,allowDeep=false){
      if(isPopout){
      ...
      if(!force && cached && cached.isConnected!==false)return cached;
      if(force)bridgeControlCache.delete(key);
      const host=bridgeRoot(cls,force,allowDeep);
      if(!host)return null;
      let control=null;
      try{if(host.matches && host.matches(selector))control=host;}catch(_){}
      if(!control){
        try{control=host.querySelector(selector);}catch(_){}
      }
      if(!control && allowDeep)control=deepQuery(host,selector);
    ...
    function bridgeReady(force=false,allowDeep=false){
      if(!bridge)return false;
      if(isPopout)return !!popoutHostConnected;
      return !!(
        bridgeControl(bridge.state,"textarea",force,allowDeep) &&
        (!bridge.command||bridgeControl(bridge.command,"textarea",force,allowDeep)) &&
        bridgeControl(bridge.run,"button",force,allowDeep) &&
        bridgeControl(bridge.stop,"button",force,allowDeep) &&
        bridgeControl(bridge.progress,"textarea",force,allowDeep)
      );
    }

    function updateKernelBadge(force=false,allowDeep=false){
      const badge=root.querySelector(".mlb-kernel-badge");
      const ready=bridgeReady(force,allowDeep);
      ...
      if(badge){
        ...
        badge.innerHTML=ready
          ...
          :"<i></i><span>Kernel Not Connected</span>";
        badge.title=ready
          ?"Run can execute this data pipeline in the Python kernel."
          :"Studio will resolve the Python bridge when an execution action is used.";
      }
      if(ready) pumpComponentImportQueue();
      return ready;
    }

    function ensureBridgeForAction(){
      if(bridgeReady(false,false))return true;
      // Deep notebook traversal is never part of initial rendering. Only a user
      // action is allowed to request this one-time compatibility fallback.
      return updateKernelBadge(true,true);
    }
    ...
    function applyExecutionProgress(next){
      ...
      if(next.runtime_kind==="train"||next.runtime_kind==="generate"){
        ...
        const reactStatusActive=runtimePanel?.tab==="status"&&runtimePanel?.mode===next.runtime_kind&&
          runtimePanel?.modelId===String(next.model_id||runtimePanel?.modelId||"");
        ...
        if(next.status==="done"||next.status==="error"||next.status==="stopped"){
          ...
          // Terminal telemetry changes the same React store. Do not throw away
          // the entire Studio DOM just to change COMPLETE/ERROR/STOPPED labels.
          if(!reactStatusActive||!refreshReactRuntimeStatus(builtModelById(runtimePanel?.modelId),next.runtime_kind))setTimeout(draw,80);
        }
      }
    ...
    function startBridgePolling(){
      ...
      bridgePollTimer=setInterval(()=>{
        ...
        if(bridgeLastReady){
          pollBridgeProgress();
          if((now-bridgeLastProbeAt)>=8000){
            bridgeLastProbeAt=now;
            updateKernelBadge(false,false);
          }
        }else if((now-bridgeLastProbeAt)>=3000){
          bridgeLastProbeAt=now;
          updateKernelBadge(false,false);
        }
      },100);
    ...
    function fullWindowPage(){
      ...
      const faviconSvg="<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"64\" height=\"64\" viewBox=\"0 0 64 64\"><image width=\"64\" height=\"64\" href=\"data:image/png;base64,\"/></svg>";
      const faviconHref="data:image/svg+xml;charset=utf-8,"+encodeURIComponent(faviconSvg);
    ...
    function openFullWindow(){
      ...
      try{
        ...
        popup.document.title="MLB Studio";
        bootstrapSent=true;
      }catch(_){ }
      ...
      const onProbeMessage=event=>{
        ...
        const onReady=readyEvent=>{
          ...
          try{
            popup.postMessage({__mlb_studio_launcher__:true,type:"bootstrap",html:page,title:"MLB Studio"},"https://builder.mlbricks.io");
            bootstrapSent=true;
            setStatus("MLB Studio opened at builder.mlbricks.io. Keep this notebook tab open for Python execution.");
...
function studioChoice(title,message,actions,options={}){
  return openStudioModal({title:title||'Choose',message:message||'',details:options.details||'',variant:options.variant||'info',actions:actions||[]});
    ...
    function openRuntimePanel(mode,entry){
      entry=liveBuiltModel(entry);
      if(!entry)return;
      if(mode==="generate"&&trainingLocksGeneration(entry)){
        setStatus("Generation is unavailable while training is updating this model. Stop or finish training first.");
        return;
      }
      ensureRuntimeConfigs(entry);
    ...
    function reactRuntimeSnapshot(entry,mode){
      entry=liveBuiltModel(entry)||entry;
      if(!entry)return null;
      ensureRuntimeConfigs(entry);
      const config=mode==="train"?entry.training_config:entry.generation_config;
      const live=mode==="train"?trainingLive(entry):generationLive(entry);
      const currentRuntimeBusy=execution.status==="running"&&
        (execution.runtime_kind==="train"||execution.runtime_kind==="generate");
      return {
        mode,
        entry:cp(entry),
        live:cp(live||{}),
        history:cp(runtimeHistory(entry,mode)||[]),
        config:cp(config||{}),
        dataset:mode==="train"?cp(preparedDatasetById(entry.selected_dataset_id)||{}):null,
        device:cp(selectedRuntimeDevice(config||{})||{}),
        valid:mode==="train"?cp(trainingConfigValid(entry,config)||{}):null,
        generationLocked:trainingLocksGeneration(entry),
        otherRuntimeBusy:currentRuntimeBusy&&execution.runtime_kind!==mode,
        execution:cp(execution||{})
      };
    }

    function reactRuntimeActions(entry,mode){
      return mode==="train" ? {
        start:()=>startTrainingFromRuntime(liveBuiltModel(entry)||entry),
        stop:requestStop,
        cleanVram:requestGpuVramCleanup,
        cancel:()=>cancelTrainingToModelEditor(liveBuiltModel(entry)||entry),
        openGeneration:()=>openRuntimePanel("generate",liveBuiltModel(entry)||entry)
      } : {
        start:()=>startGenerationFromRuntime(liveBuiltModel(entry)||entry),
        stop:requestStop,
        cancel:()=>cancelRuntimeToModelEditor(liveBuiltModel(entry)||entry,"generate")
      };
    }

    function mountReactRuntimeStatus(main,side,entry,mode){
      const api=window.__MLBReactRuntime;
    ...
    function scheduleRuntimeStatusDraw(){
      if(!runtimePanel||runtimePanel.tab!=="status")return;
      const entry=builtModelById(runtimePanel.modelId);
      if(!entry)return;
      // React status islands own hot training/generation telemetry. Each store
      // subscriber updates only its metric/text/log slice, so token/step events
      // never rebuild the Studio shell, canvas, sidebar, or focused controls.
      if(refreshReactRuntimeStatus(entry,runtimePanel.mode))return;
      if(runtimeStatusRedrawTimer)return;
    ...
    function renderRuntimeWorkspace(canvas,entry,mode){
      ...
      if(tab==="status"){
        if(!mountReactRuntimeStatus(main,side,entry,mode)){
          // Compatibility fallback for unusually old browsers where the bundled
          // React runtime could not initialize.
    ...
    function renderCentralGallery(canvas){
      ...
      const tabs=document.createElement("div");tabs.className="mlb-central-gallery-tabs mlb-gallery-flat-tabs";
      [["core","Core"],["models","Models"],["mine","My Models"],["components","Components"],["data","Data"],["drafts","Drafts"]].forEach(([key,label])=>{
      ...
      const navTools=document.createElement("div");navTools.className="mlb-gallery-flat-nav-tools";
      tabsRow.append(tabs,navTools);
      ...
      if(["core","models","mine"].includes(galleryWorkspace.tab)){
        body.classList.add("models-tab");

        // Step 10N — Core, Models and My Models are first-class Gallery tabs.
        // There is no nested Models -> Core/Models/My Models navigation.
        const modelNavTools=navTools;
        ...
        }else if(galleryWorkspace.tab==="models"){
    ...
    function requestCustomAPIImport(def,step=null){
      ...
      if(binding.call_type==="user_function"){
        ...
        if(!ensureBridgeForAction()){setStatus("Kernel bridge is offline. Re-run the Builder cell, then validate the function again.");return;}
      ...
      if(binding.call_type==="user_class"){
        ...
        if(!ensureBridgeForAction()){setStatus("Kernel bridge is offline. Re-run the Builder cell, then validate the class again.");return;}
      ...
      if(!ensureBridgeForAction()){setStatus("Kernel bridge is offline. Re-run the Builder cell, then test the API again.");return;}
    ...
    // Left/right have a fixed universal socket at 50%, so custom terminals use
    // the four reserved slots around it instead of ever occupying the center.
    // The occupied sequence (custom + fixed center) therefore stays evenly
    // spaced at 10% steps inside the same 30–70% bounded region.
    const CUSTOM_SIDE_TERMINAL_SLOTS={
      1:[40],
      2:[40,60],
      3:[30,40,60],
      4:[30,40,60,70]
    ...
    function centeredSideCustomTerminalPercent(index,count){
      const total=Math.max(1,Math.min(CUSTOM_TERMINAL_LIMIT_PER_SIDE,Number(count)||1));
      const slots=CUSTOM_SIDE_TERMINAL_SLOTS[total]||CUSTOM_SIDE_TERMINAL_SLOTS[4];
    ...
    function customTerminalStyle(item){
      ...
      // Left/right custom terminals never occupy the universal 50% center
      // socket, so they stay on the normal card edge without a second column.
    ...
    function portButtons(node, side){
      ...
      // active. Top/bottom use the centered 30–70% region; left/right reserve a
    ...
    function drawUnsafe(force=false){
      ...
      rememberWorkspaceView();
      // Full shell redraws are now reserved for explicit navigation/editor
      // structure changes. Unmount any React runtime islands cleanly first.
      try{window.__MLBReactRuntime?.unmountInstance?.(reactInstanceId);}catch(_){}
      root.innerHTML="";
      ...
      const logoBrand=document.createElement("img");logoBrand.className="mlb-logo-brand";logoBrand.alt="MLBRICKS STUDIO";logoBrand.src="data:image/webp;base64,";logo.appendChild(logoBrand);
      const title=document.createElement("div");
      ...
      if(current(state)?.kind!=="custom_edit"){
        if(runtimeWorkspaceActive){
          ...
          galleryBtn.addEventListener("click",()=>{
            ...
            openGallery(state.active_workspace==="data"?"data":"core");
          });
      ...
      if(galleryWorkspace.open){
        const gname=document.createElement("div");gname.className="mlb-runtime-toolbar-name";
        const galleryNames={core:"Core",models:"Models",mine:"My Models",components:"Components",data:"Data",drafts:"Drafts"};
        gname.textContent=galleryNames[galleryWorkspace.tab]||"Gallery";toolbar.appendChild(gname);
      ...
      if(!galleryWorkspace.open && !cloudWorkspace.open && !runtimeWorkspaceActive){
        ...
        const renderConnections=()=>{
          ...
          // transform:scale changes pixels but not layout. Update wrapper geometry
          // only when its size actually changed; writing the same size can still
          // trigger expensive notebook layout/ResizeObserver work.
          const baseW=Math.max(flow.scrollWidth,flow.offsetWidth);
          const baseH=Math.max(flow.scrollHeight,flow.offsetHeight);
          const nextW=Math.ceil(baseW*zoom)+"px";
          const nextH=Math.ceil(baseH*zoom)+"px";
          if(wrap.style.width!==nextW)wrap.style.width=nextW;
          if(wrap.style.height!==nextH)wrap.style.height=nextH;
          drawEdges(wrap,flow);
          const pos=workspaceScroll[state.active_workspace]||{left:0,top:0};
          if(canvas.scrollLeft!==(pos.left||0))canvas.scrollLeft=pos.left||0;
      ...
      if(bottomView==="outputs"){
        ...
        if(state.active_workspace==="data"){
          ...
          p1.querySelector(".mlb-preset-card").addEventListener("click",()=>openGallery(state.active_workspace==="data"?"data":"core"));
...
window.__MLB_STUDIO_GET_JS_SOURCE__=function(){
  if(!window.__MLB_STUDIO_JS_SOURCE__){
    window.__MLB_STUDIO_JS_SOURCE__=(window.__MLB_REACT_BOOTSTRAP_SOURCE__||"")+"("+__MLB_STUDIO_FACTORY__.toString()+")();";
  }
  return window.__MLB_STUDIO_JS_SOURCE__;
};
